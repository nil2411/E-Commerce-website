...
import { backendUrl } from '../constants2'
...
const normalizeCategory = (raw) => {
  ...
  return (
    ALLOWED_CATEGORIES.find((c) => c.toLowerCase() === s.toLowerCase()) ?? 'Men'
  )
}
...
const normalizeSubCategory = (raw) => {
  const s = typeof raw === 'string' ? raw.trim() : ''
  return (
    ALLOWED_SUBCATEGORIES.find((c) => c.toLowerCase() === s.toLowerCase()) ??
    'Topwear'
  )
}
...
const Add = ({ token }) => {
  ...
  const editProduct = location.state || null
  const editId = editProduct?._id
  ...
  useEffect(() => {
    ...
    const cat = editProduct.category ?? editProduct.Category
    const sub =
      editProduct.subCategory ??
      editProduct.subcategory ??
      editProduct.SubCategory
    setCategory(normalizeCategory(typeof cat === 'string' ? cat : ''))
    setSubCategory(normalizeSubCategory(typeof sub === 'string' ? sub : ''))
    setPrice(editProduct.price ?? "");
    setStock(editProduct.stock ?? 0);
    setSku(editProduct.sku ?? "");
    setBestseller(!!editProduct.bestseller);
    setSizes(Array.isArray(editProduct.sizes) ? editProduct.sizes : []);
  }, [editProduct]);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = editId
        ? await axios.post(`${backendUrl}/products/update`, formData, { headers: { token } })
        : await axios.post(`${backendUrl}/products/add`, formData, { headers: { token } })
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3 '>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          ...
          <input onChange={(e) => setStock(e.target.value)} value={stock} className='w-full px-3 py-2 sm:w-[120px]' type="number" min="0" step="1" required />
        </div>
        <div>
          <p className='mb-2'>SKU</p>
          <input onChange={(e) => setSku(e.target.value)} value={sku} className='w-full px-3 py-2 sm:w-[150px]' type="text" placeholder="SKU-001" />
        </div>
      </div>
